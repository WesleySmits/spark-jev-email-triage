 ...
 *
 * The inbox is given the stored-judgment lookup, so the thread a body read
 * returns decides whether what was judged still holds. Reading mail stays
 * read-only: the lookup opens the local database read-only and classifies
 * nothing.
 */
...
import { verifiedClassificationFor } from './stored-classifications.server'
...
export function sparkInbox() {
  inbox ??= createLiveInbox({
    reader: sparkReader(),
    timeZone: timeZone(),
    verify: (observed) => verifiedClassificationFor(observed),
  })
  return inbox
}
